 ...
 */
    ...
    struct q_learning
    {
        ...
        void operator()(state_class state, action_class action, state_class next,
                        policy<state_class, action_class> &policy_map, bool terminate);
    };
 ...
 ********************************************************************************/
    ...
    value_type policy<state_class, action_class, value_type>::best_value(state_class s_t)
    {
        ...
        return it != __policies__[s_t].end() ? it->second : std::numeric_limits<value_type>::quiet_NaN();
    }
    ...
    policy<state_class, action_class, value_type>::best_action(state_class s_t)
    {
        auto it = std::max_element(__policies__[s_t].begin(),
                                   ...
                                   [&](const auto &lhs, const auto &rhs)
                                   { return lhs.second < rhs.second; });
        return it != __policies__[s_t].end() ? std::move(std::make_unique<action_class>(it->first)) : nullptr;
    }
    ...
    policy<state_class, action_class, value_type>::best(state_class s_t)
    {
        auto it = std::max_element(__policies__[s_t].begin(),
                                   ...
                                   [&](const auto &lhs, const auto &rhs)
                                   { return lhs.second < rhs.second; });
        return it != __policies__[s_t].end() ? std::make_pair(std::make_unique<action_class>(it->first), it->second) : std::make_pair(nullptr, std::numeric_limits<value_type>::quiet_NaN());
    }

    template <class state_class,
              class action_class,
              typename value_type>
    void policy<state_class, action_class, value_type>::clear()
    {
        __policies__.clear();
    }
    ...
    template <class state_class>
    std::size_t hasher<state_serial<state_class>>::operator()(const state_serial<state_class> &arg) const
    {
        return arg.hash();
    }

    template <class action_class>
    std::size_t hasher<action_serial<action_class>>::operator()(const action_serial<action_class> &arg) const
    {
        return arg.hash();
    }
#endif
    ...
    q_learning<state_class, action_class, markov_chain, value_type>::q_value(markov_chain &episode,
                                                                             unsigned int index,
                                                                             policy<state_class, action_class> &policy_map)
    {
    ...
    template <class state_class,
              ...
              typename value_type>
    typename q_learning<state_class, action_class, markov_chain, value_type>::triplet
    q_learning<state_class, action_class, markov_chain, value_type>::q_value(state_class state, action_class action, state_class next,
                                                                             policy<state_class, action_class> &policy_map, bool terminate)
    {
        if (terminate)
        {
        ...
        else
        {
            ...
            auto r = state.reward();
            //std::cout << "q: " << q << " q_next: " << q_next << " r: " << r << "\n";
            //std::cout << "dir: " << action.trait().dir << "\n";
            if (std::isnan(q_next))
                q_next = 0.;
    ...
    template <class state_class,
              ...
              typename markov_chain,
              typename value_type>
    void q_learning<state_class, action_class, markov_chain, value_type>::operator()(markov_chain episode,
                                                                                     policy<state_class, action_class> &policy_map)
    {
    ...
    template <class state_class,
              ...
              typename markov_chain,
              typename value_type>
    void q_learning<state_class, action_class, markov_chain, value_type>::operator()(state_class state, action_class action, state_class next,
                                                                                     policy<state_class, action_class> &policy_map, bool terminate)
    {
        auto triplet = q_value(state, action, next, policy_map, terminate);
        policy_map.update(std::get<0>(triplet),
                          std::get<1>(triplet),
                          std::get<2>(triplet));
    }
    ...
    template <class state_class,
              ...
              typename markov_chain,
              typename value_type>
    q_probabilistic<state_class, action_class, markov_chain, value_type>::q_probabilistic(value_type discount)
        : gamma(discount)
    {
    }
    ...
    q_probabilistic<state_class, action_class, markov_chain, value_type>::q_value(markov_chain &episode,
                                                                                  unsigned int index,
                                                                                  policy<state_class, action_class> &policy_map)
    {
        auto step = episode[index];
    ...
    template <class state_class,
              ...
              typename markov_chain,
              typename value_type>
    void q_probabilistic<state_class, action_class, markov_chain, value_type>::operator()(markov_chain episode,
                                                                                          policy<state_class, action_class> &policy_map)
    {
        for (unsigned int i = 0; i < episode.size(); i++)
        {
